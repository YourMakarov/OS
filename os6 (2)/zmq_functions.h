...
void send_message(zmq::socket_t &socket, const std::string &msg) {
    ...
    socket.send(message, zmq::send_flags::none); //возврат результата отсылает очередь сообщений созданных в message
}
...
std::string receive_message(zmq::socket_t &socket) {
    ...
    //int chars_read;
    //try {
     //   chars_read = (int)socket.recv(message, zmq::recv_flags::none); //получение сообщений
    //}
    //catch (...) {
     //   chars_read = 0;
    //}
    //if (chars_read == 0) {
     //   return "Error ......";
    //
    zmq::recv_result_t ret(socket.recv(message, zmq::recv_flags::none));
